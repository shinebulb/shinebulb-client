...
function ThemeCard({ id, index, bg, font, title, savedList, setSavedList, settings, setSettings }) {
    ...
    return (
        <div className="theme-card" style={{backgroundColor: bg}}>
            <p style={{color: font}}>{title || `${text[lang(settings.language)].themeCard[0]} #${index + 1}`}</p>
            <div className="saved-controls">
                <button
                    title={text[lang(settings.language)].themeCard[1]}
                    style={{
                        backgroundColor: bg,
                ...
                <button
                    title={text[lang(settings.language)].themeCard[2]}
                    style={{
                        backgroundColor: bg,
                        border: `${font} 3px solid`
                    }}
                ...
                <button
                    title={text[lang(settings.language)].themeCard[3]}
                    style={{
                        backgroundColor: bg,
                        border: `${font} 3px solid`
                    }}
                ...
                <dialog ref={renameRef} className="confirm">
                    <input type="text" ref={inputRef} placeholder={text[lang(settings.language)].savedDialogs[0]}/>
                    <button onClick={renameTheme}>{text[lang(settings.language)].themeControls[0]}</button>
                    <button onClick={() => renameRef.current.close()}>{text[lang(settings.language)].themeControls[1]}</button>
                </dialog>
                <dialog ref={deleteRef} className="confirm">
                    <p>{text[lang(settings.language)].savedDialogs[1]}</p>
                    <button onClick={deleteTheme}>{text[lang(settings.language)].confirm[1]}</button>
                    <button onClick={() => deleteRef.current.close()}>{text[lang(settings.language)].confirm[2]}</button>
                </dialog>
            </div>
        </div>
    )
