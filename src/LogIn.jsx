...
function LogIn({ bulb, settings, setSettings, setSavedList }) {
    ...
    useEffect(() => {
        document.title = text[lang(settings)].auth[0];
    }, []);
    ...
    return (
        ...
        >
            {!authState.status ?
            <>
                <h2 style={{width: "80vw"}}>{text[lang(settings)].login[0]}</h2>
                <div className="login-warning" style={{display: warningDisplay}}>
                    <p>{text[lang(settings)].logInWarning[0]}</p>
                    <div>
                        <button onClick={() => hideWarning(false)}>{text[lang(settings)].logInWarning[1]}</button>
                        <button onClick={() => hideWarning(true)}>{text[lang(settings)].logInWarning[2]}</button>
                    </div>
                </div>
                <div className="login-form">
                    ...
                    <input
                        ...
                        value={username}
                        placeholder={text[lang(settings)].login[1]}
                    />
                    <div>
                    ...
                    <input
                        ...
                        value={password}
                        placeholder={text[lang(settings)].login[2]}
                    />
                    <button type="submit" onClick={login}>{text[lang(settings)].auth[0]}</button>
                </div>
                ...
                >
                    <div>
                        <p>{text[lang(settings)].authNotifications[errorText]}</p>
            ...
            : <>
                <h2>{text[lang(settings)].authErrors[4]}</h2>
            </>}
        </motion.div>
    )
}
